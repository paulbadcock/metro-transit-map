/* ─── Route 194 Bus Tracker — Frontend ──────────────────────────────────────── */
...
function loadSettings() {
  const raw = localStorage.getItem('rt194_settings');
  return raw ? { ...DEFAULTS, ...JSON.parse(raw) } : { ...DEFAULTS };
}

function saveSettings(s) {
  localStorage.setItem('rt194_settings', JSON.stringify(s));
}
...
function makeBusIcon(bearing, timestamp) {
  const rotation = bearing != null ? `transform:rotate(${bearing + 90}deg)` : '';
  let badge = '';
  if (timestamp) {
...
async function fetchVehicles() {
  state.vehicles = await apiFetch(`/api/vehicles?route_id=${encodeURIComponent(currentRouteId())}`);
...
async function fetchTripUpdates() {
  state.allTripUpdates = await apiFetch(`/api/trip-updates?route_id=${encodeURIComponent(currentRouteId())}`);
}

async function fetchStops() {
  state.stops = await apiFetch(`/api/stops?route_id=${encodeURIComponent(currentRouteId())}`);
  populateStopDropdowns();
  drawStopMarkers();
}

async function fetchRoutePolyline() {
  const data = await apiFetch(`/api/route-stops?route_id=${encodeURIComponent(currentRouteId())}`);
  drawRoutePolylines(data);
}

async function fetchScheduleForStop(stopId) {
  if (!stopId) { state.schedule = []; return; }
  state.schedule = await apiFetch(`/api/schedule?stop_id=${encodeURIComponent(stopId)}&route_id=${encodeURIComponent(currentRouteId())}`);
}
...
async function fetchRoutes() {
  ...
  sel.innerHTML = routes.map(r => {
    const label = r.route_long_name ? `${r.route_short_name} \u2014 ${r.route_long_name}` : r.route_short_name;
...
function buildBusPopup(v) {
  ...
  const ts = v.timestamp ? new Date(v.timestamp * 1000).toLocaleTimeString('en-CA', { timeZone: HALIFAX_TZ, hour: '2-digit', minute: '2-digit' }) : '–';
  const ageS = v.timestamp ? Math.round(Date.now() / 1000 - v.timestamp) : null;
  const ageText = ageS != null ? ` <span style="opacity:0.65">(${ageS}s ago)</span>` : '';
  return `
...
function drawRoutePolylines(directions) {
  ...
  directions.forEach((dir, i) => {
    ...
    const latlngs = (dir.shape && dir.shape.length > 1)
      ? dir.shape
      : (dir.stops || []).filter(s => s.stop_lat && s.stop_lon).map(s => [s.stop_lat, s.stop_lon]);
...
function updateRouteHeader() {
  document.querySelector('#logo span:last-child').textContent = `Route ${currentRouteId()}`;
...
async function switchRoute() {
  ...
  try { await fetchStops(); } catch (err) { console.warn('[BusTracker] Could not load stops:', err); }
  try { await fetchRoutePolyline(); } catch (err) { console.warn('[BusTracker] Could not load route polyline:', err); }
  try { await fetchVehicles(); } catch (err) { console.error('[BusTracker] Vehicle fetch failed:', err); updateStatusDot(false); }
  try { await fetchTripUpdates(); } catch (err) { console.warn('[BusTracker] Trip update fetch failed:', err); }

  const stopId = getActiveStop();
  if (stopId) {
    try { await fetchScheduleForStop(stopId); } catch {}
  }
...
function updateCommutePanel() {
  ...
  if (window === 'morning') {
    ...
    badge.textContent = stopId ? 'Outside commute window' : 'Configure stops in Settings';
...
function updateNextBuses(stopId) {
  ...
  if (upcoming.length === 0) {
    el.innerHTML = '<div class="empty-state">No upcoming buses in the next 2 hours.</div>';
    return;
  }
  ...
  for (const s of upcoming) {
    if (s.minutesAway <= threshold && s.minutesAway >= 0) {
      ...
      if (!state.notifiedTripIds.has(key)) {
        ...
        const stopName = state.stops.find(x => x.stop_id === stopId)?.stop_name || 'your stop';
        const departBy = minutesToTimeString(nowMinutes() + Math.round(s.minutesAway));
        fireNotification(
...
window.testNotification = () => {
  Notification.requestPermission().then(perm => {
    if (perm === 'granted') {
      fireNotification('Route 194 test', 'This is a test notification from Route 194 Bus Tracker.');
    } else {
...
document.getElementById('settings-form').addEventListener('submit', async (e) => {
  ...
  settings.notifMinutes = parseInt(document.getElementById('notif-minutes').value);
...
async function checkServiceStatus() {
  try {
    const s = await apiFetch(`/api/service-status?route_id=${encodeURIComponent(currentRouteId())}`);
...
async function init() {
  ...
  setInterval(async () => {
    ...
    if (sid) {
      try { await fetchScheduleForStop(sid); } catch {}
      updateCommutePanel();
    }
  }, 5 * 60_000);
